...
# features = ['user_rating_mean', 'user_votes_mean', 'extend', 'product_votes_mean']
features = ['user_rating_mean', 'user_votes_mean', 'user_helpful_votes_mean'] \
        + ['product_id', 'product_rating_mean', 'product_votes_mean','product_helpful_votes_mean'] \
        + ['user_votes_helpful_rate', 'user_helpful_rating'] \
        + ['product_votes_helpful_rate', 'product_helpful_rating'] \
        # + ['user_rating_std', 'user_rating_var', 'product_rating_std', 'product_rating_var'] \
...
# 早停的参数
patience = 10  # 设置耐心，表示如果经过 `patience` 个 epoch 没有提升则停止训练
...
for epoch in range(n_epochs):
    ...
    print(f"Epoch [{epoch+1}/{n_epochs}], Train Loss: {running_loss/len(train_loader):.4f}, Validation Loss: {avg_val_loss:.4f}, RMSE Loss: {rmse:.4f}")
...
with torch.no_grad():
    ...
    outputs = model(inputs)
    # test_data['rating'] =  np.round(np.clip(outputs.cpu().numpy(), 0, 5))
    test_data['rating'] =  outputs.cpu().numpy()
